...
@app.post("/upload")
async def upload_video(file: Optional[UploadFile] = File(None), instructions: str = Form("")):
    file_id = uuid.uuid4().hex
    has_file = file is not None and file.filename

    if not has_file and not instructions.strip():
        return JSONResponse(
            status_code=400,
            content={"error": "Please provide either a recording file or instructions/notes."}
        )
    ...
    try:
        ...
        if has_file:
            ...
            if file_ext not in VIDEO_EXTENSIONS + AUDIO_EXTENSIONS:
                return JSONResponse(
                    status_code=400,
                    content={"error": f"Unsupported file type: {file_ext}. Supported: {VIDEO_EXTENSIONS + AUDIO_EXTENSIONS}"}
                )
        ...
        docx_filename = f"GHL_Technical_Approach_{file_id}.docx"
...
async def download_file(filename: str):
    ...
    return FileResponse(
        path=str(file_path),
        filename="GHL_Solution_Architect.docx",
